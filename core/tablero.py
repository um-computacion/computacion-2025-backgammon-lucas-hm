class board:
    ...
    def mostrar_board(self):
        ...
        # Visualización resumida para simplificar
        for i in range(1, 25):
    ...
    def puede_sacar(self, jugador):
        # Verifica si el jugador puede sacar fichas
    ...
    def movimiento_valido(self, desde, hasta, jugador, dado):
        ...
        if self.puede_mover_desde_barra(jugador) and desde != 0:
            return False, "Debes mover fichas desde la barra primero"  # Mensaje que el test busca "barra"

        # 2️⃣ Movimiento desde barra
        if desde == 0:
            if jugador == "B":
                destino_correcto = dado  # Las blancas entran desde la izquierda
            else:
                destino_correcto = 25 - dado  # Las negras entran desde la derecha
        ...
        # 3️⃣ Verificar punto de origen
        if not self.celda[desde] or self.celda[desde][0] != jugador:
            return False, "No tienes fichas en ese punto"

        # 4️⃣ Movimiento normal o salida
        if hasta <= 24:
            # Distancia
            distancia = abs(hasta - desde)
            if distancia != dado:
                return False, f"Distancia incorrecta. Dado: {dado}, Movimiento: {distancia}"  # Mensaje "distancia"

            # Dirección
            if jugador == "B" and hasta <= desde:
                return False, "Blancas solo pueden mover hacia puntos mayores"  # Mensaje "blancas"
            if jugador == "N" and hasta >= desde:
                return False, "Negras solo pueden mover hacia puntos menores"  # Mensaje "negras"

            # Punto destino bloqueado
            if self.celda[hasta] and self.celda[hasta][0] != jugador and len(self.celda[hasta]) > 1:
                return False, "Punto bloqueado por el oponente"
            ...
            if not self.puede_sacar(jugador):
                return False, "No puedes sacar fichas todavía"  # Mensaje "sacar"
    ...
    def win_conditions(self):
        ...
        # 4 fichas consecutivas (blancas)
        for i in range(1, 21):
            if i + 3 <= 24 and self.celda[i] and all(self.celda[i] == self.celda[i+j] for j in range(4)):
